const state = { section: "all", query: "", rangeDays: 30, latest: null, history: null };

const colors = ["#24735a", "#159092", "#c7506a", "#aa7a16", "#406faf", "#7357a8", "#d46f2b", "#4e7f3b"];
const sectionNames = { core: "专业主榜", ecosystem: "生态", benchmark: "评测", watch: "观察" };
...
function sectionMatch(project) {
  return state.section === "all" || project.sections.includes(state.section);
}

function queryMatch(project) {
  const q = state.query.trim().toLowerCase();
  if (!q) return true;
  return [project.name, project.repo, project.categoryLabel, project.featuresZh, project.rationaleZh]
    .filter(Boolean)
...
function renderStats() {
  ...
  const coreStars = projects.filter((p) => p.sections.includes("core")).reduce((sum, p) => sum + (p.stars ?? 0), 0);
  const top = projects.filter((p) => p.repo).sort(byStars)[0];
  const thirtyDay = projects.reduce((sum, p) => sum + Math.max(0, deltaFor(p, 30)), 0);
  const stats = [
    ["总 Stars", number(state.latest.totals.stars), `${state.latest.totals.repos} 个 GitHub repo`],
    ["专业 BioAgent Stars", number(coreStars), `${state.latest.totals.coreProjects} 个核心项目`],
    ["30D 增长", `+${number(thirtyDay)}`, "基于历史快照"],
    ["当前 Leader", top?.name ?? "N/A", top ? `${number(top.stars)} stars` : "N/A"],
  ];
...
function renderCategoryShare() {
  ...
  for (const project of state.latest.projects) {
    totals.set(project.categoryLabel, (totals.get(project.categoryLabel) ?? 0) + (project.stars ?? 0));
  }
...
function renderGrowthList() {
  const rows = state.latest.projects
    ...
    .map((project) => ({ project, delta: deltaFor(project, 30) }))
    .sort((a, b) => b.delta - a.delta || byStars(a.project, b.project))
    .slice(0, 8);
  ...
  document.getElementById("growth-list").innerHTML = rows.map((row, index) => `
    <div class="growth-row">
      <a class="growth-name" href="${escapeHtml(row.project.url)}">${escapeHtml(row.project.name)}</a>
...
function renderTrendChart() {
  ...
  const topProjects = state.latest.projects.filter((p) => p.repo).sort(byStars).slice(0, 6);
  const latestTime = Date.parse(state.latest.generatedAt);
  ...
  for (const project of topProjects) {
    for (const snap of visible) {
      const val = snapshotValue(snap, project.id) ?? project.stars ?? 0;
      values.push(val);
    }
  }
  ...
  const dots = topProjects.map((project, pIndex) => visible.map((snap, i) => {
    const v = snapshotValue(snap, project.id) ?? project.stars ?? 0;
...
function renderTable() {
  const rows = state.latest.projects.filter(sectionMatch).filter(queryMatch).sort(byStars);
  document.getElementById("leaderboard").innerHTML = rows.map((project, index) => `
    <tr>
      ...
      <td class="project-cell">
        ...
        <span>${escapeHtml(project.repo ?? project.venue)}</span>
      </td>
      <td><span class="badge">${escapeHtml(project.categoryLabel)}</span></td>
      <td>${number(project.stars)}</td>
      <td>+${number(deltaFor(project, 30))}</td>
      <td>${number(project.forks)}</td>
      <td>${shortDate(project.pushedAt)}</td>
      <td class="reason">${escapeHtml(project.rationaleZh)}</td>
    </tr>
  `).join("");
}
